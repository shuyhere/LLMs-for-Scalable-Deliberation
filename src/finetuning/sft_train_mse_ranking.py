#!/usr/bin/env python3
"""
MSE + Ranking Loss Training for Multi-output Regression
Focuses on MSE loss and pairwise ranking loss ONLY (no correlation loss).

KEY FEATURES:
1. Dual Loss Function:
   - MSE loss for absolute value accuracy
   - Pairwise ranking loss for ordering accuracy
   
2. No correlation loss to prevent early stopping
3. Stronger ranking loss component for better ordering
"""
...
class MSERankingModel(nn.Module):
    """Model with MSE + Ranking loss (NO correlation loss)"""
    def __init__(self, base_model_name: str, num_dims: int = 4, 
                 dropout_rate: float = 0.2, hidden_dropout: float = 0.3,
                 mse_weight: float = 0.5, rank_weight: float = 0.5):
        super().__init__()
        ...
        # Loss weights (MSE + Ranking only, NO correlation)
        self.mse_weight = mse_weight
        self.rank_weight = rank_weight
        
        print(f"Model initialized with MSE weight: {mse_weight}, Ranking weight: {rank_weight}")
    ...
    def forward(self, input_ids=None, attention_mask=None, labels=None, **kwargs):
        ...
        if labels is not None:
            ...
            # 2. Pairwise Ranking Loss (for ordering accuracy)
            ranking_losses = []
            ...
            for i in range(batch_size):
                for j in range(i + 1, batch_size):
                    for dim in range(self.num_dims):
                        ...
                        # Only consider pairs with meaningful difference
                        label_diff = label_i - label_j
                        if abs(label_diff) > 0.02:  # Threshold for meaningful difference
                            pred_diff = pred_i - pred_j
                            
                            # MarginRankingLoss: correct ordering should have same sign
                            # If label_i > label_j, pred_i should > pred_j
                            target = torch.sign(label_diff)
                            
                            # Hinge loss with margin
                            margin = 0.05  # Smaller margin for smoother training
                            loss_val = F.relu(margin - target * pred_diff)
                            ranking_losses.append(loss_val)
            ...
            # Combined loss (NO correlation loss)
            loss = self.mse_weight * mse_loss + self.rank_weight * ranking_loss
            
            # Log for monitoring
            if self.training:
                self.last_mse = mse_loss.item()
                self.last_ranking = ranking_loss.item()
    ...
    def on_log(self, args, state, control, model=None, logs=None, **kwargs):
        if model is not None and hasattr(model, 'module'):
            ...
            if hasattr(base_model, 'last_mse'):
                ...
                self.loss_history['ranking'].append(base_model.last_ranking)
                
                if len(self.loss_history['mse']) % 10 == 0:
                    avg_mse = np.mean(self.loss_history['mse'][-10:])
                    avg_rank = np.mean(self.loss_history['ranking'][-10:])
                    print(f"\n📊 Loss components (last 10 steps): MSE={avg_mse:.4f}, Ranking={avg_rank:.4f}")
...
def main():
    parser = argparse.ArgumentParser(description="MSE + Ranking loss training (NO correlation loss)")
    ...
    parser.add_argument("--wandb-project", type=str, default="llm_mse_ranking")
    parser.add_argument("--wandb-run-name", type=str, default="mse-ranking-deberta")
    ...
    print("\n" + "="*60)
    print("MSE + RANKING LOSS TRAINING")
    print("="*60)
    print(f"NO correlation loss in training")
    print(f"MSE weight: {args.mse_weight}, Ranking weight: {args.rank_weight}")
    ...
    print("\n📊 Computing Ordering Accuracy...")
    try:
        device = next(trainer.model.parameters()).device
        ordering_results = compute_ordering_accuracy(eval_ds, trainer.model, device)
        metrics.update(ordering_results)
    except Exception as e:
        print(f"Error computing ordering accuracy: {e}")
